...
void WriteCsv(const std::string& path, const cs223::common::RunConfig& cfg, const std::string& workload_name,
  ...
  out << "workload,cc,threads,duration_s,p_hot,hotset_size,committed,aborted,retries,throughput_tps,avg_latency_ms,p50_ms,p95_ms,p99_ms,balance_before,balance_after\n";
  out << workload_name << ',' << cc_name << ',' << cfg.threads << ',' << cfg.duration_s << ',' << cfg.p_hot << ','
      ...
      << ',' << throughput << ',' << (avg_latency_s * 1000.0) << ','
